...
@admin.action(
    description="Remote delete selected installations (uninstalls from GitHub)"
)
...
@admin.action(
    description="Verify with GitHub and mark missing installations as remote-deleted"
)
def verify_with_github_bulk(modeladmin, request, queryset):
    for installation in queryset.filter(remote_deleted_at__isnull=True):
        try:
            ...
            logger.exception(
                "Failed to verify installation %s with GitHub",
...
class InstallationAdmin(ModelAdmin):
    ...
    def remote_delete_detail(self, request, object_id):
        ...
        if not installation:
            return redirect(
                reverse("admin:github_app_installation_change", args=[object_id])
            )
        if installation.remote_deleted_at:
            self.message_user(
                request, "Installation is already remote-deleted.", messages.WARNING
            )
        else:
            installation.uninstall()
            self.message_user(
                request,
                "Installation uninstalled from GitHub and marked as remote-deleted.",
                messages.SUCCESS,
            )
        return redirect(
            reverse("admin:github_app_installation_change", args=[object_id])
    ...
    def verify_with_github_detail(self, request, object_id):
        ...
        if installation.remote_deleted_at:
            self.message_user(
                request,
                "Installation is already marked as remote-deleted locally.",
                messages.WARNING,
            )
        else:
            try:
                check_installation_active(installation.installation_id)
                self.message_user(
                    request, "Installation is still active on GitHub.", messages.SUCCESS
                )
            except InstallationNotFoundError:
                installation.mark_remote_deleted()
                self.message_user(
